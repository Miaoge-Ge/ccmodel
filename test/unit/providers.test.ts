...
import { parseStream, flattenMessages, toolMarkerInstructions } from "../../src/providers/cursorClient.js";
...
test("cursor flattenMessages separates system from a labeled transcript", () => {
  const { system, transcript } = flattenMessages([
  ...
  assert.ok(transcript.includes("USER: hello"));
  assert.ok(transcript.includes("ASSISTANT: hi"));
  assert.ok(transcript.includes("ASSISTANT (called tools)"));
  assert.ok(transcript.includes("TOOL RESULT (c1)"));
});

test("cursor toolMarkerInstructions lists tools, or is empty when none", () => {
  assert.equal(toolMarkerInstructions([]), "");
  const instr = toolMarkerInstructions([{ type: "function", function: { name: "alpha" } }, { name: "beta" }]);
  assert.ok(instr.includes("alpha") && instr.includes("beta"));
  assert.ok(instr.includes("CLAUDE_TOOL_CALL"));
});
